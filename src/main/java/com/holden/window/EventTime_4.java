...
public class EventTime_4 {
    public static void main(String[] args) throws Exception {
        ...
        /*dataStream.keyBy(SensorReading::getId).window(TumblingEventTimeWindows.of(Time.seconds(10)))
                .reduce(new ReduceFunction<SensorReading>() {
                    ...
                    public void process(String s, ProcessWindowFunction<SensorReading, String, String, TimeWindow>.Context context, Iterable<SensorReading> elements, Collector<String> out) throws Exception {
                        SensorReading next = elements.iterator().next();
                        out.collect("全窗口聚合:[" + context.window().getStart() + "," + context.window().getEnd() + ")" + next.toString());
                    }
                }).print();*/
        //开窗口再聚合通过id聚合一次,这样就不分窗口了
        sumStream.keyBy(SensorReading::getId).sum("temperature").print();
