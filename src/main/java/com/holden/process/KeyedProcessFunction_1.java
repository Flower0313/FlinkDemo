...
public class KeyedProcessFunction_1 {
    public static void main(String[] args) throws Exception {
        ...
        OutputTag<SensorReading> outputTag = new OutputTag<SensorReading>("nihao") {
        };


        //step-2 逻辑操作
        SingleOutputStreamOperator<SensorReading> process = dataStream.keyBy(SensorReading::getId)
                .process(new MyProcess());

        process.getSideOutput(outputTag).print("s2>>>>>>");
        process.print("other>>>>>>");
     ...
     * */
    public static class MyProcess extends KeyedProcessFunction<String, SensorReading, SensorReading> {

        //step-1 定义单值状态
        private ValueState<Long> tsTimerState;
        private static final Timer time = new Timer();
        private final OutputTag<SensorReading> noMatch = new OutputTag<SensorReading>("nihao") {
        };
        ...
        public static class TimerTaskTest extends java.util.TimerTask {
            private Collector<SensorReading> out;
            private SensorReading value;
            private KeyedProcessFunction<String, SensorReading, SensorReading>.Context ctx;
            private static OutputTag<SensorReading> nono= new OutputTag<SensorReading>("nihao"){};

            public TimerTaskTest(Collector<SensorReading> out, KeyedProcessFunction<String, SensorReading, SensorReading>.Context ctx, SensorReading value) {
                this.out = out;
                this.ctx = ctx;
                this.value = value;
            }
            ...
            @Override
            public void run() {
                System.out.println("ctx-->>" + ctx + ",value-->>" + value);
                ctx.output(nono,value);
        ...
        public void processElement(SensorReading value, KeyedProcessFunction<String,
                SensorReading, SensorReading>.Context ctx,
            ...
            if (value.getId().equals("s2")) {
                time.schedule(new TimerTaskTest(out, ctx,value), 5000);
            } else {
                out.collect(value);
            }
        ...
        public void onTimer(long timestamp, KeyedProcessFunction<String, SensorReading, SensorReading>.OnTimerContext ctx, Collector<SensorReading> out) throws Exception {
             ...
             *  水位线为什么能自己间歇性生成呢？这是使用定时器做到的，在定时器的onTimer方法中又注册一个定时，套娃来执行
             * */
            ctx.timerService().registerProcessingTimeTimer(ctx.timerService().currentProcessingTime() + 1000L);
